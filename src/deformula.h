...
const static double pi = 4.0 * std::atan(1.0);
...
double DeformulaZeroToInf::phi(double t) const {
  return std::exp(pi * std::sinh(t) / 2.0);
}

double DeformulaZeroToInf::phidash(double t) const {
  return pi * std::cosh(t) * std::exp(pi * std::sinh(t) / 2.0) / 2.0;
}
...
double DeformulaMinusOneToOne::phi(double t) const {
  return std::tanh(pi * std::sinh(t) / 2.0);
}

double DeformulaMinusOneToOne::phidash(double t) const {
  return pi * std::cosh(t) * (1.0 / std::cosh(pi * std::sinh(t) / 2.0)) * (1.0 / std::cosh(pi * std::sinh(t) / 2.0)) / 2.0;
}
}
